...
import {ILockToVetoPlugin} from "../src/ILockToVetoPlugin.sol";
...
contract LockToVetoPluginTest is Test {
    address immutable daoBase = address(new DAO());
    address immutable pluginBase = address(new LockToVetoPlugin());
    address immutable votingTokenBase = address(new ERC20VotesMock());

    DAO public dao;
    LockToVetoPlugin public plugin;
    ...
    function setUp() public {
        ...
        LockToVetoPlugin.OptimisticGovernanceSettings
            memory settings = LockToVetoPlugin.OptimisticGovernanceSettings({
                minVetoRatio: uint32(RATIO_BASE / 10),
                minDuration: 10 days,
                minProposerVotingPower: 0
            });

        plugin = LockToVetoPlugin(
            createProxyAndCall(
                address(pluginBase),
                abi.encodeWithSelector(
                    LockToVetoPlugin.initialize.selector,
                    dao,
                    settings,
                    votingToken
                )
    ...
    function test_InitializeSetsTheProperValues() public {
        ...
        assertEq(plugin.totalVotingPower(), 10 ether, "Incorrect token supply");
        assertEq(
            plugin.minVetoRatio(),
    ...
    function test_SupportsIOptimisticTokenVotingInterface() public {
        bool supported = plugin.supportsInterface(
            type(ILockToVetoPlugin).interfaceId
        );
        ...
        public
    {
        ...
        IDAO.Action[] memory actions = new IDAO.Action[](0);
        LockToVetoPlugin.OptimisticGovernanceSettings
            memory newSettings = LockToVetoPlugin.OptimisticGovernanceSettings({
                minVetoRatio: uint32(RATIO_BASE / 10),
                minDuration: 10 days,
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LockToVetoPlugin.ProposalCreationForbidden.selector,
                bob
            )
        );
        plugin.createProposal("", actions, 0, 0, 0);
    ...
    function test_CreateProposalRevertsIfThereIsNoVotingPower() public {
        ...
        // Try to create
        vm.expectRevert(
            abi.encodeWithSelector(LockToVetoPlugin.NoVotingPower.selector)
        );
        plugin.createProposal("", actions, 0, 0, 0);
    }
        ...
        public
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LockToVetoPlugin.DateOutOfBounds.selector,
                startDate + 10 days,
                endDate
            )
        );
    ...
    function test_CreateProposalStartsNowWhenStartDateIsZero() public {
        ...
        (
            ,
            ,
            LockToVetoPlugin.ProposalParameters memory parameters,
            ,
            ,
    ...
    function test_GetProposalReturnsTheRightValues() public {
        ...
        actions[0].data = abi.encodeWithSelector(
            LockToVetoPlugin.totalVotingPower.selector,
            0
        );
    ...
    function test_CanVetoReturnsFalseWhenAVoterAlreadyVetoed() public {
        ...
        votingToken.approve(address(plugin), 1);
    ...
    function test_VetoRevertsWhenAProposalDoesntExist() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LockToVetoPlugin.ProposalVetoingForbidden.selector,
                proposalId + 200,
                alice
            )
        );
    ...
    function test_VetoEmitsAnEvent() public {
        ...
        vm.expectEmit();
        emit VetoCast(proposalId, alice, 1);
        plugin.veto(proposalId, 1);
    }
    ...
    function test_ExecuteRevertsWhenNotEnded() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LockToVetoPlugin.ProposalExecutionForbidden.selector,
                proposalId
            )
        );
        plugin.execute(proposalId);
        ...
        public
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                LockToVetoPlugin.MinDurationOutOfBounds.selector,
                4 days,
                4 days - 1
            )
        );
        ...
        // 2
        newSettings = LockToVetoPlugin.OptimisticGovernanceSettings({
            minVetoRatio: uint32(RATIO_BASE / 10),
    ...
    function test_UpgradeToRevertsWhenCalledFromNonUpgrader() public {
        address _pluginBase = address(new LockToVetoPlugin());
    ...
    function test_UpgradeToAndCallRevertsWhenCalledFromNonUpgrader() public {
        ...
        plugin.upgradeToAndCall(
            _pluginBase,
            abi.encodeWithSelector(
                LockToVetoPlugin.updateOptimisticGovernanceSettings.selector,
                settings
            )
        );
    }
